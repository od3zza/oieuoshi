...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.addEventListener('click', (event) => {
        // Verifica se o clique não foi no painel e nem no botão de abrir
        if (!tagsSidebar.contains(event.target) && !openTagsButton.contains(event.target) && tagsSidebar.classList.contains('open')) {
            tagsSidebar.classList.remove('open');
        }
    });

    // --- Lógica para buscar e gerar as tags do highlights.json ---
    async function fetchAndGenerateTags() {
        try {
            ...
            // Coleta todas as tags de todos os itens em 'data' (seu array de objetos)
            const allTags = new Set(); // Usamos um Set para garantir tags únicas
            data.forEach(item => {
                if (item.tags && Array.isArray(item.tags)) {
                    item.tags.forEach(tag => allTags.add(tag.trim())); // Adiciona tags, removendo espaços extras
                }
            });

            const uniqueSortedTags = Array.from(allTags).sort((a, b) => a.localeCompare(b)); // Converte para array e ordena

            generateTags(uniqueSortedTags); // Chama a função para renderizar as tags
        } catch (error) {
            console.error('Erro ao buscar ou processar as tags:', error);
            tagsList.innerHTML = '<li>Erro ao carregar tags.</li>'; // Mensagem de erro para o usuário
        }
    }
    ...
    function generateTags(tagsArray) {
        ...
        tagsArray.forEach(tagText => {
            ...
            const tagElement = document.createElement('span'); // Você pode usar 'a' se elas forem links navegáveis
            tagElement.classList.add('tag');
            tagElement.textContent = tagText;
            tagElement.dataset.tag = tagText.toLowerCase().replace(/\s/g, '-');

            tagElement.addEventListener('click', () => {
                // Lógica para ativar/desativar a tag (se for um filtro, por exemplo)
                const isActive = tagElement.classList.toggle('active');
                
                // Se você quiser que apenas uma tag fique ativa por vez:
                // if (isActive) {
                //     document.querySelectorAll('.tags-list .tag').forEach(t => {
                //         if (t !== tagElement) t.classList.remove('active');
                //     });
                // }

                console.log(`Tag clicada: ${tagText}, Ativa: ${isActive}`);
                // **Aqui você adicionaria sua lógica de filtragem de conteúdo**
                // Por exemplo, chamando uma função para filtrar seus artigos
            });
    ...
    // Chamar a função para buscar e gerar as tags
    fetchAndGenerateTags();
});
