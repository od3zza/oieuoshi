...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const tagsList = document.getElementById('tags-list'); // Novo local para as tags
    ...
    // Fechar o painel ao clicar fora dele (opcional, mas boa prática)
    document.addEventListener('click', (event) => {
        // Se o clique não foi no painel e nem no botão de abrir, feche o painel
        if (!tagsSidebar.contains(event.target) && !openTagsButton.contains(event.target) && tagsSidebar.classList.contains('open')) {
            tagsSidebar.classList.remove('open');
        }
    });

    // --- Lógica para gerar as tags (Exemplo) ---
    // Adapte esta parte do seu JS existente que gera as tags.
    // Agora, em vez de `tags-container`, você vai usar `tags-list`.
    const availableTags = ['HTML', 'CSS', 'JavaScript', 'Web Design', 'Acessibilidade', 'Performance', 'SEO', 'React', 'Vue.js', 'Angular', 'Python', 'Node.js', 'Banco de Dados', 'Design UX/UI'];

    function generateTags(tagsArray) {
        tagsList.innerHTML = ''; // Limpa a lista existente antes de adicionar novas tags
        tagsArray.forEach(tagText => {
            const listItem = document.createElement('li');
            const tagElement = document.createElement('span'); // Ou 'a' se for um link
            tagElement.classList.add('tag');
            tagElement.textContent = tagText;
            tagElement.dataset.tag = tagText.toLowerCase().replace(/\s/g, '-'); // Exemplo de data-attribute

            // Adicione aqui a lógica para lidar com o clique na tag, se necessário
            // Por exemplo, para filtrar conteúdo:
            tagElement.addEventListener('click', () => {
                // Remove a classe 'active' de todas as tags
                document.querySelectorAll('.tag').forEach(t => t.classList.remove('active'));
                // Adiciona 'active' à tag clicada
                tagElement.classList.add('active');
                console.log(`Tag clicada: ${tagText}`);
    ...
    // Chama a função para gerar as tags quando a página carregar
